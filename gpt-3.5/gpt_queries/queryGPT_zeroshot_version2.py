# This file is used for 0-shot prompting
...
# Results are stored in folder data2/zero_shot/gpt-model-number. GPT output is stored in csv file named "newstance_combined_0shot_gpt{model}_all2_domain.csv"
# confusion matrix and accuracy/precision/recall/f1 for those results is stored in "conf_matrix_newstance_combined_0shot_gpt{model}_all2_domain.csv"
...
def prompt_generation():
    ...
    for file in os.listdir(file_path):
        if file.endswith("all2_domain.csv") and file != "out_domain.csv": #todo: change back to go through all files
            domain = file
            ...
            df_testing = df.iloc[15:] #todo: change this to exclude the first ??? so that we always test on the same set 
            ...
            combined_export_path = export_path + "newstance_combined_0shot_gpt4o_" + domain
...
def shot_query(domain,prompts):
    ...
    for index,prompt in enumerate(prompts):
        if requests>0 and requests%50 == 0:
            print("Sleeping for 20 seconds...") #prob not needed for 3.5
            time.sleep(20)
...
#can improve robustnese of this function anc parse the response to automate the metrics calculations
