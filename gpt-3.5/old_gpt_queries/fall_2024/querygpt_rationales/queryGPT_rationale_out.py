...
def prompt_generation():
    ...
    def one_shot(num_shots=1):
        ...
        for file in os.listdir(file_path):
            if file.endswith(".csv") and (file in ['health_domain.csv','humans_domain.csv']):
                ...
                df_full = pd.read_csv(f_name)
                # Aggregate out-domain data
                selected_training, training_abstracts = aggregate_out_domain_data(domain, 1)
                df_testing = df_full.tail(len(df_full) - 4)
                #prompt generation
                f_idx = f_name.replace('.csv','_idx.csv')
                df_domain_indexed = pd.read_csv(f_idx)
    ...
    def aggregate_out_domain_data(current_domain, num_shots):
        # Iterate over all files and collect first four rows from other domains
        for file in os.listdir(file_path):
            if file.endswith(".csv") and file != current_domain and (file in ['health_domain.csv','humans_domain.csv']):
                df = pd.read_csv(file_path + file)
                # Add the first four rows to the out-domain training pool
                df2 = pd.read_csv(file_path + file.replace('.csv','_idx.csv'))
                df_out_domain_data = df.head(4)
                # Randomly select between 1 and num_shots samples from the out-domain training data
                selected_training  = df_out_domain_data.sample(n=num_shots, random_state=42)
                ...
                return selected_training, training_abstracts
    ...
    def n_CoT(num_shots):
        ...
        for file in os.listdir(file_path):
            if file.endswith(".csv") and (file in ['health_domain.csv','humans_domain.csv']):
                ...
                df_full = pd.read_csv(f_name)
                # Aggregate out-domain data
                selected_training, training_abstracts = aggregate_out_domain_data(domain, num_shots)
                df_testing = df_full.tail(len(df_full) - 4)
                #prompt generation
                f_idx = f_name.replace('.csv','_idx.csv')
                df_domain_indexed = pd.read_csv(f_idx)
